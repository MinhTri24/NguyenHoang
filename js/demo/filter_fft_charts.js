function updateFFTChart() {
  ...
  async function fetchData() {
    const url = 'http://192.168.1.16:3000/tanthuan/fft';
  ...
  fetchData().then((data) => {
    ...
    // Create arrays for fft values (fft 1 to fft 4)
    const fftValues = [];
    for (let i = 1; i <= 6; i++) {
      const fieldKey = `FFT${i}`;
      const fftData = slicedData.map(feed => feed[fieldKey]);
      //fftData.reverse();
      fftValues.push(fftData);
    }
    ...
    // Create an array of datasets for each fft
    const datasets = fftValues.map((fftData, index) => ({
      data: fftData,
      label: `FFT ${index + 1}`,
    }));

    // Set the labels of all datasets to "FFT 1" to "FFT 6"
    const datasetLabels = datasets.map((dataset, index) => `FFT ${index + 1}`);
    datasets.forEach(dataset => (dataset.label = datasetLabels.shift()));

    const selectedDatasetIndex = parseInt(document.getElementById('dataset-select-fft').value);
    const selectedDataset = datasets[selectedDatasetIndex];

    const filteredFFTData = {
      labels: created_at,
      datasets: [selectedDataset],
    };

    // Update the chart with the filtered fft data
    myFFTChart.data = filteredFFTData;
    myFFTChart.update();
  });
}
// Add an event listener to the "Update Timestamp" button
const updateFFTTimestampButton = document.getElementById('update-fft-timestamp-button');
updateFFTTimestampButton.addEventListener('click', updateFFTChart);
...
setInterval(function () {
  ...
  document.getElementById('fft-last-updated').innerHTML = `Updated ${formattedDate}`;
  updateFFTChart();
}, 100000);
// setup 
const fft_data = {
...
// render init block
const myFFTChart = new Chart(
  document.getElementById('myFFTChart'),
  fft_config_filter
);
...
// function exportfftData(chart) {
//   const data = chart.data.datasets[0].data;
//   const labels = chart.data.labels;
//   //let csvContent = "data:text/csv;charset=utf-8,";
//   csvContent += "Date,US#1\n";

//   data.forEach((fft, index) => {
//       csvContent += labels[index] + "," + fft + "\n";
//   });

//   const filename = "fft_data.csv";
//   downloadCSV(csvContent, filename);
// }
function getEnglishLabel(vietnameseLabel) {
  const labelNumber = vietnameseLabel.match(/\d+/);
  return labelNumber ? `FFT${labelNumber}` : '';
}
...
document.getElementById("exportFFTData").addEventListener("click", function () {
  exportFFTData(myFFTChart); // Replace 'fftChart' with the variable name of your ffterature chart instance
});
...
document.getElementById("saveFFTImage").addEventListener("click", function () {
  const now = new Date();
  ...
  saveChartAsImage(myFFTChart, `fft_chart_${formattedDate}.png`);
